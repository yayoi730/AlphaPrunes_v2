...
def minimax(curr_board, curr_list_c_boards, last_move, depth, alpha, beta, ally, start_time):
    """
    Minimax
...
def max_value(curr_board, curr_list_c_boards, last_move, depth, alpha, beta, ally, start_time):
    """
...
def min_value(curr_board, curr_list_c_boards, last_move, depth, alpha, beta, ally, start_time):
    """
    ...
    for move in list_of_possible_moves:
        ...
        try:
            max_move, score = minimax(u_curr_board, u_curr_list_c_boards, chosen_move, depth - 1, alpha, beta,
                                      not ally, start_time)  # currently ally = true, false(ally)) = False = Enum
...
def points_from_move(move, curr_board, curr_list_of_c_boards, ally):
    updated_board, updated_list_of_c_boards = update_board(curr_board, curr_list_of_c_boards, move, ally)
    return points_won(updated_board, updated_list_of_c_boards)
...
def partition(moves_list, g_board_pos, low, high, curr_board, curr_list_of_c_boards, ally):
    ...
    for j in range(low, high):
        if points_from_move([g_board_pos, moves_list[j]], curr_board, curr_list_of_c_boards, ally) <= pivot:
            ...
            (moves_list[i], moves_list[j]) = (moves_list[j], moves_list[i])

    # Swap the pivot element with the greater element specified by i
    (moves_list[i + 1], moves_list[high]) = (moves_list[high], moves_list[i + 1])
...
def sort_move_list(moves_list, g_board_pos, low, high, curr_board, curr_list_of_c_boards, ally):
    if low < high:
        # Find pivot element such that
        # element smaller than pivot are on the left
        # element greater than pivot are on the right
        pi = partition(moves_list, g_board_pos, low, high, curr_board, curr_list_of_c_boards, ally)

        # Recursive call on the left of pivot
        sort_move_list(moves_list, g_board_pos, low, pi - 1, curr_board, curr_list_of_c_boards, ally)

        # Recursive call on the right of pivot
        sort_move_list(moves_list, g_board_pos, pi + 1, high, curr_board, curr_list_of_c_boards, ally)


def shave_move_list(sorted_moves_list, shave_num):
    if len(sorted_moves_list) > 4:  # if length of possible moves is greater than 4
        sorted_list_len = len(sorted_moves_list)
